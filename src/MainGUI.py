...
from PyQt5.QtWidgets import QFileSystemModel,  QVBoxLayout,QAction, QWidget, QFileDialog, QDialog, QAbstractItemView, QPushButton, QListWidgetItem, QGridLayout, QScrollArea
...
class aboutGUI(QDialog):
    def __init__(self, parent=None):
        ...
        self.dialog = uic.loadUi('res/pages/About.ui', self)
...
class MouseObserver(QObject):
    def __init__(self, widget):
        """
        Initializes the MouseObserver class.

        Args:
            widget (QWidget): The widget to observe for mouse events.
        """
    ...
    def eventFilter(self, obj, event):
        """
    Filters events for the observed widget.
    ...
    Returns:
        int: The current mouse state (1 for pressed, 0 for released).
    """
        #print(self.mouse)
        return self.mouse    
...
class App(QtWidgets.QMainWindow):
    ...
    def __init__(self):
        ...
        self.ui = uic.loadUi('res/pages/ObjDet.ui', self)
    ...
    def createMenubar(self):
        ...
        propertiesAction = QAction("Set New Project Directory", self)
        propertiesAction.triggered.connect(lambda: self.resetProjectDir())
        self.ui.menuProperties.addAction(propertiesAction)
    ...
    def projectPage(self):
        ...
        if self.persistance.contains('project folder'):
            ...
            grid_layout.setHorizontalSpacing(40)  # Increase horizontal spacing
            grid_layout.setVerticalSpacing(40) 
            
            if self.persistance.value('project folder') == '':
                return
            
        
            for index, dir in enumerate(os.listdir(self.persistance.value('project folder'))):
                self.folders[dir] = QPushButton(dir)
                self.folders[dir].setFixedSize(150, 150)
                self.folders[dir].setStyleSheet("background-color:#6793b5; font-size: 16px;")
            ...
            listItem = QListWidgetItem(self.model_list)
            listItem.setSizeHint(scroll_area.sizeHint())
            self.model_list.setItemWidget(listItem, scroll_area)
    
        else:
            self.btn  = QPushButton("Set Project Directory")
            
            self.buttonWidget = QWidget()
            layout = QVBoxLayout(self.buttonWidget)
            layout.addWidget(self.btn)
            layout.setAlignment(Qt.AlignCenter)
            layout.setContentsMargins(0, 0, 0, 0)
            self.buttonWidget.setLayout(layout)
                
            listItem = QListWidgetItem(self.model_list)
            self.model_list.setItemWidget(listItem, self.buttonWidget)
            #self.model_list.addItem(self.btn)
            #self.model_list.addItem(self.btn, 1, 3, 2,1,)
